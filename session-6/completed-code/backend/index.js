...
// for a mobile app, we can't actually test with https because it requires a REAL certificate authority to generate a certificate and key for us
// the command below generates a self-signed certificate that can work great for testing on web, but not for testing with mobile
// mobile OS require verified certificates, so for the sake of time I will leave the code here, but the certificate is something you'd have to learn to generate
...
// if using HTTPS instead, change "app" -> "server"
app.listen(8000, () => {
...
app.delete("/songs/delete/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const deleteSong = await pool.query("DELETE FROM songs WHERE id = $1", [
      id,
    ]);
...
app.get("/liked/song", async (req, res) => {
  ...
  try {
    const checkLikes = await pool.query(
      "SELECT COUNT(*) > 0 AS has_liked FROM song_likes WHERE song_id = $1 AND user_id = $2",
      [songId, userId]
    );
...
app.post("/songs/like", async (req, res) => {
  ...
  try {
    ...
    if (hasLiked) {
      ...
      await pool.query("UPDATE songs SET likes = likes - 1 WHERE id = $1", [
        songId,
      ]);
    } else {
      await pool.query(
    ...
    const updatedPost = await pool.query("SELECT * FROM songs WHERE id = $1", [
      songId,
    ]);
